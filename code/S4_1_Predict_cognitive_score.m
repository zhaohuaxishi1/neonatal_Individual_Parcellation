%% predict_cognitive_score.m
% Predict 18-month cognitive outcome using Ti-PCA + SVR framework
% Author: jlzhao

clear; clc; tic;

%% --------------------------- Configuration ---------------------------
addpath(genpath('utils/SVR'));  % Add SVR tools to path
...
clear V_vector_age PredictionCognitionScore;

%% --------------------------- Prediction ---------------------------
Prediction = SVR_NFolds_Topography_PCA(...
    SubjectsData, Cognition, fold_quantity, pre_method, c_para, ...
...
fprintf('[Cognitive Prediction] r = %.3f, p = %.3g\n', r_value, p_value);
...
for i = 1: 100
    W_Calculate_SVR_Random(i, SubjectsData, Cognition, fold_quantity, pre_method, c_para, n_networks, n_voxels_per_net, network_order, weight_dir);
end
